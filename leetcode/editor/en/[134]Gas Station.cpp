...
Therefore, you can't travel around the circuit once no matter where you start.
 ...
 Related Topics Array Greedy 👍 7643 👎 708
...
{
public:
    //Tip:Save the stretch with the biggest deficit for last
    int canCompleteCircuit(vector<int> &gas, vector<int> &cost)
    {
        int ans = 0, gas_surplus = 0, min_gas_surplus = 0;
        ...
        for (int i = 0; i < gas.size(); ++i)
        {
            ...
            if (min_gas_surplus > gas_surplus)
            {
                min_gas_surplus = gas_surplus;
                ans = (i == gas.size()) ? 0 : i + 1;
            }
        }
        return (gas_surplus < 0) ? -1 : ans;
    }
};
