...
//The maximum difference is max(|nums[1] - nums[4]|, |nums[2] - nums[5]|) = max(
//0, 1) = 1. Therefore, we return 1.
// 
//
// Example 2: 
//
...
// 
//
// Related Topics Array Binary Search Greedy 👍 1412 👎 128
#include<vector>
#include <algorithm>
...
public:
    int minimizeMax(vector<int> &nums, int p)
    {
        ...
        while (left < right)
        {
            const int max_difference = (left + right) / 2;
            if (numPairsIsSufficient(nums, max_difference, p))
            {
    ...
    static bool numPairsIsSufficient(const vector<int> &nums, int diff, int p)
    {
        int pairs = 0;
